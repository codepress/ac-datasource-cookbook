...
- [`Facade\Relation\Table::has_one()`](../api/facades.md#acadatasourcesfacaderelationtable)
  — joins two Data Sources on a foreign-key column; both sides share the
  same row when queried.
- [`Facade\Relations`](../api/facades.md#acadatasourcesfacaderelations)
  — the container that holds one or more relations for a Data Source.
...
!!! tip "When to use `has_many` instead"
...
```php title="examples/04-table-relation.php"
--8<-- "examples/04-table-relation.php"
```
